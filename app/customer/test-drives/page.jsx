...
export default function TestDrivesPage() {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      const response = await fetch("http://192.168.1.19:8080/api/user/test-drive/list", {
        method: "POST",
        headers: {
  ...
  const handleCancel = async (id) => {
    try {
      ...
      const response = await fetch(`http://192.168.1.19:8080/api/user/test-drive/${id}/cancel`, {
        method: "PUT",
        headers: {
          "Authorization": `Bearer ${token}`
        }
